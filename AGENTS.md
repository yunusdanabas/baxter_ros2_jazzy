...
- Active plan: `MASTER_PLAN.md` (steps I00–I19). This is the source of truth.
...
- The hardware gates I10, I11 and I12 have all passed (I10/I11 2026-07-22,
  I12 2026-07-24). Never enable or move the real robot outside a supervised
  session, and never expand a passed gate into a broader support claim than
  its log records.
...
Check `MASTER_PLAN.md` for the first non-`completed`, non-`blocked`,
non-`deferred` step. As of 2026-07-25 every numbered step through I20 is
`completed` except I13, which is `deferred`. Open work is publish prep (sim-first
public GitHub release): real maintainer email, private vulnerability reporting,
merge to `main`, and tag `v0.2.0` after `docs/ci_release_checklist.md` is green.
Remaining robot-session experiments (raising `path_tolerance_rad`, left/right
`w0` compare) are optional follow-ups, not blockers for a carefully worded
sim-first release. If a new step is added later, use its latest prompt in
`PROMPTS.md` (the last entry for that step supersedes earlier ones). If no
prompt exists yet for a new step, first write one from the step's
`MASTER_PLAN.md` entry using the prompt template in `WORKFLOW.md`, append it to
`PROMPTS.md`, then run it in a fresh session.
